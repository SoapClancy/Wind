...
class WindFarmDataSet(DeepLearningDataSet):
    ...
    def _shift_power_output_one_day_ahead(wind_farm_extended):
        ...
        :return:
        """
        shift = wind_farm_extended['wind speed'].shift(periods=-1, freq=datetime.timedelta(hours=1))
...
if __name__ == '__main__':
    ...
    def get_training_or_test_data_set(this_wind_farm_what, this_wind_farm_operating_regime_what):
        what_data_set = WindFarmDataSet(
            ...
            stacked_shift_size=(datetime.timedelta(hours=1),),
            how_many_stacked=(24 * 2,)
        )
    ...
    training_data_set_for_nn = training_data_set.windowed_dataset(datetime.timedelta(hours=1), batch_size=700 * 24,
                                                                  shift=1,
                                                                  drop_remainder=True)

    test_data_set = get_training_or_test_data_set(this_wind_farm_test,
                                                  this_wind_farm_operating_regime_test)
    test_data_set_for_nn = test_data_set.windowed_dataset(datetime.timedelta(hours=1), batch_size=700 * 24,
                                                          shift=1,
                                                          drop_remainder=True)
    ...
    # ####################4号模型#############################
    # model = tf.keras.models.Sequential([
    #     tf.keras.layers.RNN(ed.layers.LSTMCellFlipout(32),  # LSTMCellFlipout
    #                         # return_sequences=True,
    #                         input_shape=training_data_set_for_nn.element_spec[0].shape[1:]),
    #
    ...
    # ####################载入模型#############################
    model.load_weights(r'E:\mymodel_epoch_250.h5')
    pass
    ...
    # history = model.fit(training_data_set_for_nn, verbose=2, epochs=201, validation_data=test_data_set_for_nn,
    #                     callbacks=[CustomCallback()], )

    model.save('mymodel.h5')

    training_data_set_for_nn = training_data_set.windowed_dataset(datetime.timedelta(hours=1), batch_size=500 * 24 * 6,
                                                                  drop_remainder=True)

    test_data_set_for_nn = test_data_set.windowed_dataset(datetime.timedelta(hours=1), batch_size=500 * 24 * 6,
                                                          drop_remainder=True)
    ...
    def plot_plot_uct(day: int, test=True):
        if test:
            ...
            day_y = training_list[1][day].flatten()
        pre = np.full((50, 6), np.nan)
        for j in range(50):
            pre[j] = model(day_x[np.newaxis, ...], training=True).numpy().flatten()
        return pre.T, np.mean(pre.T, axis=1), day_y
    ...
    day_y_list = None
    for i in range(24 * 7):
        this_pre, this_pre_mean, this_day_y = plot_plot_uct(24 * 9 + i)  # 24*9
        if i == 0:
            pre_list = this_pre
    ...
    trans = load_pkl_file(
        r"C:\Users\SoapClancy\OneDrive\PhD\01-PhDProject\02-Wind\MyProject\Data\Results\Forecasting\Dalry\transformation_args\Dalry_training.pkl")
    trans = trans[0]['min_max_transformed_col']
    ax = None
    # ax = series([10000000], [1], color='g', label='Prediction uncertainty')

    ax = series(day_y_list * 31.8, color='r', linestyle='--', label='Recording', ax=ax,
                x_label='Time Stamp [10 min]',
                x_lim=(-2, 144 * 7 + 2),
                x_ticks=([x * 144 for x in range(0, 8)], [str(x * 144) for x in range(0, 8)]),
                y_label='Wind Speed [m/s]')
    ax.fill_between(x=range(pre_list.shape[0]),
                    y1=np.min(pre_list * 31.8, axis=1),
                    y2=np.max(pre_list * 31.8, axis=1), color='g', alpha=0.5,
                    label='Prediction uncertainty')
    ax = series(pre_mean_list * 31.8, color='b', ax=ax, linestyle='-', label='Prediction mean', zorder=10)
