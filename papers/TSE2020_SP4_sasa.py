...
from PowerCurve_Class_sasa import PowerCurveByMfr, PowerCurveFittedBy5PLF
...
def fit_plot_and_summary_all_mfr_pc_in_all_density(mode: str):
    ...
    for i, this_mfr_pc in enumerate(mfr_pc):
        if (i != 0) and (i != mfr_pc.__len__() - 1):
            continue
        # Fitting results' saving path
        save_to_file_path = project_path_ / f"Data/Results/PowerCurve/sasa/Mfr_fittings_{this_mfr_pc.air_density}.pkl"
        ...
        # To summarise
            ...
            if i == mfr_pc.__len__() - 1:
                error_df.to_csv('12Mfr-PC summary_sasa.csv')
                direct_cmp_df.T.to_csv('direct_cmp_df_sasa.csv')
    ...
    if mode == 'summary':
        ...
        # 1.27
        ax = series(x=wind_speed, y=mfr_pc_to_fit[-1](wind_speed), color='b', linestyle='-', ax=ax,
                    label='5PLF (' + r'$\rho$' + f'={mfr_pc[-1].air_density} kg/m' + '$^3$)', zorder=lower_layer)
        ax = scatter(x=mfr_wind_speed, y=mfr_pc[-1](mfr_wind_speed), marker='o', s=16, ax=ax,
                     rasterized=False,
                     facecolors='none', edgecolors='magenta',
                     label='Mfr-PC (' + r'$\rho$' + f'={mfr_pc[-1].air_density} kg/m' + '$^3$)', zorder=upper_layer,
                     y_lim=(-0.05, 1.05),
                     x_lim=(-0.05, 26.5),
